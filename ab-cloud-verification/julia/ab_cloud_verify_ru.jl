...
# Идиоматичный Julia · Аннотации типов · Эффективный
# =============================================================================
...
function main()
    ...
    println("  Загружено нулей: $(length(gammas)) (γ-ординаты)")

    t = time()
    results = []
    if objection == 0 || objection == 1; r = objection1(gammas); push!(results, ("1:b(N)", r[1], r[2])); end
    if objection == 0 || objection == 2; r = objection2(gammas); push!(results, ("2:KS", r[1], r[3])); end
    if objection == 0 || objection == 3; r = objection3(gammas); push!(results, ("3:slope", r[1], r[2])); end
