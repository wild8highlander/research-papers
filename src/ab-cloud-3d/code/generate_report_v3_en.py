"""
generate_report_v3 — English Version
============================================================

Generate verification report v3 with deep statistical analysis.

This is the English translation of generate_report_v3.py.
Russian comments in the code body are preserved for reference.

Original file: generate_report_v3.py
"""
...
def build_report():
    ...
    summary_text = (
        ...
        "нелинейность), проведено 24 численных эксперимента с псевдоспектральным "
        "методом 4-го порядка (N=1024, T=50). Показано, что механизм M2 "
        "(Родригес) сохраняет инварианты с точностью 10⁻⁴ — на два порядка "
        "лучше диссипативных моделей. Этот результат обобщён на mKdV, BBM, "
        "Кавахару и 2D уравнение Кадомцева–Петвиашвили (KP). В §16.24 "
        "реализована изоспектральная модификация b через поток K₂ KdV-"
        "иерархии, в §16.26 — Polchinski-style непрерывный RG-поток на K₁, "
        "стабильный для 50+ итераций (vs 2-3 для дискретного K₂). В §16.25, "
        "§16.26 показано, что эти конструкции глубоко связаны с классической "
        "ренормализацией Уилсона (β-функция ↔ K₂-поток, μ ↔ θ_b, Λ ↔ k_max/3). "
...
def build_report_part5(doc):
    ...
    rows_16_10 = [
        ...
        ["polchinski_rg.py", "Polchinski-K₁ непрерывный RG-поток (50+ шагов)", "≈ 320 строк"],
        ...
        ["generate_report.py", "Генерация DOCX отчёта", "≈ 2080 строк"],
        ["Всего", "Полная верификация + отчёт", "≈ 7460 строк"],
    ]
    add_table(doc,
              ["Файл", "Описание", "Объём"],
              rows_16_10,
