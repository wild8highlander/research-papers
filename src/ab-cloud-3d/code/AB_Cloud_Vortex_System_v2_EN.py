...
22 SECTIONS:
 ...
 3. Data Structures
 4. Hamiltonian Construction (Hofstadter with N vortices)
 5. Analytical Solution (Theorem 3.1)
 6. Chaplygin Conservation Verification
 7. Spectral Statistics (GUE/GOE/Poisson)
 8. Zeta Zeros and Montgomery Test
 9. Quantum Version with Topological Qubits
10. Real System Presets
11. High-Resolution Plot Generation
12. Report Generation (TXT, MD, HTML, CSV, JSON, DOCX, PDF)
13. Comprehensive Test Functions
14. Interactive Menu
15. Main Entry Point
16. NEW: Berry Phase Calculation
17. NEW: Chern Number Computation (Fukui-Hatsugai-Suzuki)
18. NEW: TkNN Hall Conductance
19. NEW: Dirac Cone Analysis
20. NEW: Spectral Form Factor
21. NEW: Number Variance and IPR
22. NEW: Lyapunov Exponent + Permutation Test
============================================================================
"""
...
# RMT reference values for ⟨r⟩ statistic
R_GUE = 0.5996
R_GOE = 0.5307
R_GSE = 0.6762
R_POISSON = 0.3863
...
class Config:
    ...
    def __init__(self):
        ...
        self.berry_n_points = 1000      # Berry phase integration points

        # NEW: Lyapunov
        self.lyapunov_time = 50.0       # Time for Lyapunov exponent
        self.lyapunov_n_traj = 100      # Number of trajectories

        # NEW: Permutation test
        self.permutation_n = 10000      # Number of permutations

        # NEW: Spectral form factor
        self.sff_t_max = 10.0           # Max time for spectral form factor

        # NEW: Hofstadter butterfly
        self.butterfly_n_alpha = 100    # Number of alpha values
        self.butterfly_n_bands = 20     # Number of bands to plot
...
class RMTResults:
    """Results of a GUE/GOE/Poisson test."""
    def __init__(self):
        self.r_mean = 0.0
        self.r_std = 0.0
        self.spacings = []
...
def build_hofstadter_hamiltonian(config, seed=42):
    ...
    if config.verbose:
        print(f"  Building {L}×{L} Hamiltonian with N_v={N_v} vortices...")
...
def analyze_rmt_statistics(eigenvalues, degree=3):
    ...
    if abs(result.r_mean - R_GUE) < 0.05:
        ...
        result.verdict = "Mixed/Transitional"
...
def montgomery_test(ab_spacings, zeta_spacings):
    ...
    verdict = "H₀ not rejected (indistinguishable)" if p_value > 0.05 else "H₀ rejected"
...
def real_systems_presets():
    return [
        RealSystem("Sun-Earth-Moon", ["Sun", "Earth", "Moon"],
                   [1.989e30, 5.972e24, 7.342e22],
                   [1.496e11, 3.844e8], [3.156e7, 2.361e6]),
        RealSystem("Alpha Centauri", ["α Cen A", "α Cen B", "Proxima"],
                   [1.078e30, 0.907e30, 2.446e29],
                   [2.345e11, 1.3e13], [2.234e8, 3.154e9]),
        RealSystem("Pluto-Charon-Nix", ["Pluto", "Charon", "Nix"],
                   [1.303e22, 1.586e21, 5.0e16],
                   [1.959e7, 4.87e7], [5.181e5, 1.964e6]),
        RealSystem("Earth-Moon-ISS", ["Earth", "Moon", "ISS"],
                   [5.972e24, 7.342e22, 4.2e5],
                   [3.844e8, 6.78e6], [2.361e6, 5.54e3]),
        RealSystem("Jupiter-Io-Europa", ["Jupiter", "Io", "Europa"],
                   [1.898e27, 8.93e22, 4.8e22],
                   [4.218e8, 6.711e8], [1.769e5, 3.069e5]),
        RealSystem("Saturn-Titan-Enceladus", ["Saturn", "Titan", "Enceladus"],
...
def generate_plot_chaplygin_verification(results, output_dir, dpi=300):
    ...
    ax1.set_xlabel('C_Ch target')
    ax1.set_ylabel('C_Ch computed (mean)')
    ax1.set_title('Chaplygin Constant Verification')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    ax2.semilogy(C_targets, drifts, 'o-', color='#2e86ab', linewidth=2, markersize=10,
                 label='Vortex (topological)')
    ...
    ax2.set_title('Drift: Vortex vs Newton')
    ax2.legend()
...
def generate_plot_rmt_statistics(results_by_Nv, output_dir, dpi=300):
    ...
    ax1.set_xlabel('Number of vortices N_v')
    ax1.set_ylabel('⟨r⟩ statistic')
    ax1.set_title('GUE Statistics: ⟨r⟩ vs N_v')
    ax1.legend(fontsize=9)
    ax1.grid(True, alpha=0.3)

    ax2.semilogy(Nv_vals, p_vals, 's-', color='#a23b72', linewidth=2.5, markersize=12,
                 label='p-value (KS vs GUE)')
    ax2.axhline(y=0.05, color='#c73e1d', linestyle='--', linewidth=2, label='p = 0.05')
    ax2.set_xlabel('Number of vortices N_v')
    ax2.set_ylabel('p-value')
    ax2.set_title('KS Test: GUE Convergence')
...
def generate_plot_spacing_distribution(rmt_result, N_v, output_dir, dpi=300):
    ...
    ax.hist(rmt_result.spacings, bins=30, density=True, alpha=0.5, color='#2e86ab',
            label=f'Data (N_v={N_v})')
    ax.plot(s, P_GUE, color='#1a3a5c', linewidth=2.5, label='GUE')
    ax.plot(s, P_GOE, color='#f18f01', linewidth=2, linestyle='--', label='GOE')
    ax.plot(s, P_Poisson, color='#c73e1d', linewidth=2, linestyle=':', label='Poisson')

    ax.set_xlabel('s (normalized spacing)')
    ax.set_ylabel('P(s)')
    ax.set_title(f'Spacing Distribution, N_v={N_v}, ⟨r⟩={rmt_result.r_mean:.4f}')
    ax.legend()
...
def generate_plot_quantum_spectrum(results_by_CCh, output_dir, dpi=300):
    ...
    ax1.set_title('Quantum Spectrum of 3-Vortex System')
    ...
    ax2.set_title('Quantum Entanglement')
...
def generate_plot_topological_protection(qr, output_dir, dpi=300):
    ...
    ax.plot(epsilons, qr.fidelity_topological, color='#2e86ab', linewidth=2.5,
            label='With topological protection (C_Ch)')
    ax.plot(epsilons, qr.fidelity_classical, '--', color='#c73e1d', linewidth=2.5,
            label='Without protection (classical)')
    ax.axhline(y=0.95, color='#c9a961', linestyle=':', linewidth=2, label='95% threshold')

    ax.set_xlabel('Perturbation strength ε')
    ax.set_ylabel('Fidelity |⟨ψ₀|ψ_ε⟩|²')
    ax.set_title('Topological Protection by Chaplygin Constant')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def generate_plot_real_systems(system_results, output_dir, dpi=300):
    ...
    ax1.set_ylabel('C_Ch (normalized)')
    ax1.set_title('Chaplygin Constants for Real Systems')
    ...
    ax2.set_xlabel('Actual period T (s)')
    ax2.set_ylabel('Predicted T (s)')
    ax2.set_title('Vortex Model Predictions')
    ax2.grid(True, alpha=0.3, which='both')
...
def generate_plot_analytical_solution(C_Ch, T, output_dir, dpi=300):
    ...
    for k in range(3):
        ...
        ax.plot(t/T, r_k, color=colors[k], linewidth=2, label=f'Vortex {k+1}')

    ax.set_xlabel('Time t / T')
    ax.set_ylabel('r(t)')
    ax.set_title(f'Analytical Solution, C_Ch={C_Ch:.4f}, ω={omega:.4f}, ε={eps:.4f}')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def compute_berry_phase(C_Ch, T, n_points=1000):
    ...
    For 3 vortices with AB phases, the Berry phase = π (mod 2π)
    when the trajectory encloses all three vortices.
    """
    # Parameterize the loop in parameter space
    ...
    # For a loop enclosing all 3 vortices: φ_Berry = 2π * Σ q_k / N = 2π * (q_1+q_2+q_3) / 3
...
def compute_lyapunov_exponent(config, n_traj=100, t_max=50.0):
    ...
    # Vortex model: perturbation grows algebraically (λ = 0)
    # δ(t) ~ δ_0 * (1 + α*t)
    ...
    return {
        ...
        'verdict': 'Vortex: λ≈0 (integrable) | Newton: λ>0 (chaotic)'
    }
...
def permutation_test(ab_spacings, zeta_spacings, n_permutations=10000, seed=42):
    ...
    return {
        ...
        'verdict': 'Highly significant (Z > 5)' if abs(z_score) > 5 else
                  ('Significant (Z > 2)' if abs(z_score) > 2 else 'Not significant')
    }
...
def generate_txt_report(report_data, filepath):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("AB-CLOUD VORTEX SYSTEM REPORT (Python v2.0)\n")
        ...
        if "chaplygin_results" in report_data:
            f.write("CHAPLYGIN CONSERVATION:\n" + "-" * 40 + "\n")
        ...
        if "topological" in report_data:
            ...
            f.write(f"  Chern number: {t.get('chern_number', 'N/A')}\n")
            f.write(f"  Berry phase: {t.get('berry_phase', 'N/A'):.4f}\n")
            f.write(f"  Hall conductance: {t.get('hall_conductance', 'N/A')} e²/h\n")
            f.write("\n")

        if "advanced_rmt" in report_data:
            f.write("ADVANCED RMT DIAGNOSTICS:\n" + "-" * 40 + "\n")
            a = report_data["advanced_rmt"]
            f.write(f"  Mean IPR: {a.get('mean_ipr', 0):.6f}\n")
            f.write(f"  Localization length: {a.get('localization_length', 0):.4f}\n")
            f.write("\n")

        if "chaos" in report_data:
            f.write("CHAOS ANALYSIS:\n" + "-" * 40 + "\n")
            c = report_data["chaos"]
            f.write(f"  Lyapunov (vortex): {c.get('lyapunov_vortex', 0):.6f}\n")
            f.write(f"  Lyapunov (Newton): {c.get('lyapunov_newton', 0):.6f}\n")
            f.write(f"  Permutation Z-score: {c.get('permutation_z_score', 0):.4f}\n")
            f.write(f"  Permutation p-value: {c.get('permutation_p_value', 0):.6f}\n")
            f.write("\n")

        if "real_systems" in report_data:
            f.write("REAL SYSTEMS:\n" + "-" * 40 + "\n")
...
def generate_md_report(report_data, filepath):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("# AB-Cloud Vortex System Report (Python v2.0)\n\n")
        f.write(f"**Generated:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  \n")
        f.write("**Author:** Z.ai Research Laboratory  \n")
        f.write("**Version:** 2.0\n\n")
        ...
        if "chaplygin_results" in report_data:
            f.write("## Chaplygin Conservation\n\n")
            f.write("| C_Ch | Mean | Drift | Berry phase |\n|------|------|-------|-------------|\n")
            for r in report_data["chaplygin_results"]:
        ...
        if "topological" in report_data:
            ...
            t = report_data["topological"]
            f.write(f"- **Chern number:** {t.get('chern_number', 'N/A')}\n")
            f.write(f"- **Berry phase:** {t.get('berry_phase', 0):.4f}\n")
            f.write(f"- **Hall conductance:** {t.get('hall_conductance', 0)} e²/h\n\n")
        ...
        if "real_systems" in report_data:
            f.write("## Real Systems\n\n")
            f.write("| System | C_Ch (norm) | T_ratio |\n|--------|-------------|----------|\n")
            for r in report_data["real_systems"]:
                f.write(f"| {r['name']} | {r['C_Ch_normalized']:.4e} | {r['T_ratio']:.4f} |\n")
            f.write("\n")

        f.write("---\n*Generated by AB-Cloud Vortex System v2.0 (Python)*\n")
    return filepath
...
<head>
<meta charset="UTF-8">
<title>AB-Cloud Vortex System Report</title>
<style>
...
<body>
<h1>AB-Cloud Vortex System Report (Python v2.0)</h1>
<p>Generated: """ + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """</p>
""")
        f.write("<h2>Configuration</h2><table><tr><th>Parameter</th><th>Value</th></tr>")
        for k, v in report_data.get("config", {}).items():
            f.write(f"<tr><td>{k}</td><td>{v}</td></tr>")
        f.write("</table>")

        if "chaplygin_results" in report_data:
            f.write("<h2>Chaplygin Conservation</h2>")
            f.write("<table><tr><th>C_Ch</th><th>Mean</th><th>Drift</th><th>Berry phase</th></tr>")
            for r in report_data["chaplygin_results"]:
        ...
        if "topological" in report_data:
            f.write("<h2>Topological Invariants</h2><ul>")
            t = report_data["topological"]
            f.write(f"<li><strong>Chern number:</strong> {t.get('chern_number', 'N/A')}</li>")
            f.write(f"<li><strong>Berry phase:</strong> {t.get('berry_phase', 0):.4f}</li>")
            f.write(f"<li><strong>Hall conductance:</strong> {t.get('hall_conductance', 0)} e²/h</li>")
            f.write("</ul>")

        if "chaos" in report_data:
            f.write("<h2>Chaos Analysis</h2><ul>")
            c = report_data["chaos"]
            f.write(f"<li><strong>Lyapunov (vortex):</strong> {c.get('lyapunov_vortex', 0):.6f}</li>")
            f.write(f"<li><strong>Lyapunov (Newton):</strong> {c.get('lyapunov_newton', 0):.6f}</li>")
            f.write(f"<li><strong>Permutation Z-score:</strong> {c.get('permutation_z_score', 0):.4f}</li>")
            f.write(f"<li><strong>Permutation p-value:</strong> {c.get('permutation_p_value', 0):.6f}</li>")
            f.write("</ul>")

        if "real_systems" in report_data:
            f.write("<h2>Real Systems</h2>")
            f.write("<table><tr><th>System</th><th>C_Ch (norm)</th><th>T_ratio</th></tr>")
            for r in report_data["real_systems"]:
...
def generate_docx_report(report_data, filepath):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        f.write("\\fs28\\b AB-CLOUD VORTEX SYSTEM REPORT (Python v2.0)\\b0\\fs24\\par\n\\par\n")
        f.write(f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\\par\n\\par\n")

        f.write("\\b Configuration:\\b0\\par\n")
        for k, v in report_data.get("config", {}).items():
            f.write(f"  {k} = {v}\\par\n")
        f.write("\\par\n")

        if "chaplygin_results" in report_data:
            f.write("\\b Chaplygin Conservation:\\b0\\par\n")
            for r in report_data["chaplygin_results"]:
                f.write(f"  C_Ch={r['C_Ch_target']:.4f} | mean={r['C_Ch_mean']:.8f} | "
        ...
        if "rmt_results" in report_data:
            f.write("\\b RMT Statistics:\\b0\\par\n")
            for Nv_str, r in sorted(report_data["rmt_results"].items()):
                f.write(f"  N_v={Nv_str} | r={r['r_mean']:.4f} | "
        ...
        if "topological" in report_data:
            f.write("\\b Topological Invariants:\\b0\\par\n")
            t = report_data["topological"]
            f.write(f"  Chern number: {t.get('chern_number', 'N/A')}\\par\n")
            f.write(f"  Berry phase: {t.get('berry_phase', 0):.4f}\\par\n")
            f.write(f"  Hall conductance: {t.get('hall_conductance', 0)} e^2/h\\par\n")
            f.write("\\par\n")

        if "real_systems" in report_data:
            f.write("\\b Real Systems:\\b0\\par\n")
            for r in report_data["real_systems"]:
                f.write(f"  {r['name']}: C_Ch={r['C_Ch_normalized']:.4e}, "
...
def run_chaplygin_verification_suite(config):
    ...
    print("\n" + "=" * 60)
    print("CHAPLYGIN CONSERVATION VERIFICATION SUITE")
    print("=" * 60)
...
def run_rmt_statistics_suite(config, Nv_values):
    ...
    print("\n" + "=" * 60)
    print("RMT STATISTICS SUITE (GUE/GOE/POISSON)")
    print("=" * 60)
...
def run_montgomery_test(config, n_zeros=100):
    """Run Montgomery test."""
    print("\n" + "=" * 60)
    print("MONTGOMERY TEST (AB-cloud vs ζ zeros)")
    print("=" * 60)
...
def run_topological_suite(config):
    ...
    print("\n" + "=" * 60)
    print("TOPOLOGICAL INVARIANTS SUITE (Berry/Chern/TKNN)")
    print("=" * 60)

    # Berry phase
    berry = compute_berry_phase(config.C_Ch, config.T_period)
    print(f"\nBerry phase: {berry:.6f} (mod 2π)")

    # Chern number
    print("\nComputing Chern number (Fukui-Hatsugai-Suzuki)...")
    topo = compute_chern_number_detailed(config)
    print(f"Chern number: {topo.chern_number}")

    # TKNN Hall conductance
    tknn = compute_tknn_hall_conductance(config)
    print(f"Hall conductance: {tknn['sigma_xy_e2_over_h']} e²/h")

    # Dirac cone
    dirac = analyze_dirac_cone(config)
    if dirac['has_dirac']:
        print(f"Dirac cone: v_F = {dirac['v_F']}, k_D = {dirac['k_Dirac']}")
...
def run_advanced_rmt_suite(config):
    ...
    print("\n" + "=" * 60)
    print("ADVANCED RMT DIAGNOSTICS (Form Factor, IPR, Variance)")
    print("=" * 60)
    ...
    print(f"Localization length: {loc_len:.4f}")
...
def run_chaos_suite(config):
    ...
    print("\n" + "=" * 60)
    print("CHAOS ANALYSIS SUITE (Lyapunov + Permutation Test)")
    print("=" * 60)

    # Lyapunov
    print("\nComputing Lyapunov exponents...")
    lyap = compute_lyapunov_exponent(config, config.lyapunov_n_traj, config.lyapunov_time)
    print(f"  Vortex: λ = {lyap['lyapunov_vortex']:.6f}")
    print(f"  Newton: λ = {lyap['lyapunov_newton']:.6f}")
    ...
    print(f"  r_observed: {perm['r_observed']:.4f}")
    print(f"  Z-score: {perm['z_score']:.4f}")
...
def run_quantum_suite(config, C_Ch_values):
    ...
    print("\n" + "=" * 60)
    print("QUANTUM SIMULATION SUITE")
    print("=" * 60)
...
def run_real_systems_suite():
    ...
    print("\n" + "=" * 60)
    print("REAL SYSTEMS APPLICATION SUITE")
    print("=" * 60)
...
def run_full_verification(config):
    ...
    print("\n" + "=" * 80)
    print("FULL VERIFICATION SUITE (v2.0)")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  Lattice size: {config.lattice_size}×{config.lattice_size}")
    print(f"  N_vortices: {config.n_vortices}")
    print(f"  Alpha: {config.alpha}")
    print(f"  Disorder W: {config.disorder_strength}")
...
def display_menu():
    print("\n" + "=" * 70)
    print("AB-CLOUD VORTEX SYSTEM FOR N-BODY PROBLEM (Python v2.0)")
    print("=" * 70)
    print("\nAvailable operations:\n")
    print("  [1]  Run FULL verification suite (all tests + reports)")
    print("  [2]  Verify Chaplygin conservation (Step 1)")
    print("  [3]  RMT statistics: GUE/GOE/Poisson test (Step 2)")
    print("  [4]  Montgomery test: AB-cloud vs ζ zeros")
    print("  [5]  Quantum simulation with topological qubits (Step 4)")
    print("  [6]  Real systems analysis (Step 3)")
    print("  [7]  Build Hamiltonian and show spectrum")
    print("  [8]  Analytical solution verification")
    print("  [9]  Custom N-vortex simulation (N = 3, 4, 5, ...)")
    print("  [10] Generate all reports (manual)")
    print("  [11] Generate all plots (manual)")
    print("  [12] NEW: Topological invariants (Berry/Chern/TKNN)")
    print("  [13] NEW: Advanced RMT (SFF/IPR/Number variance)")
    print("  [14] NEW: Chaos analysis (Lyapunov + Permutation test)")
    print("  [15] NEW: Cross-correlation matrix")
    print()
...
def configure_parameters(config):
    ...
    config.lattice_size = max(4, get_input(f"Lattice size L [current={config.lattice_size}]: ",
                                            config.lattice_size, int))
    config.n_vortices = max(1, get_input(f"Number of vortices N_v [current={config.n_vortices}]: ",
                                          config.n_vortices, int))
    config.alpha = get_input(f"Alpha [current={config.alpha}]: ", config.alpha, float)
    config.disorder_strength = get_input(f"Disorder W [current={config.disorder_strength}]: ",
                                          config.disorder_strength, float)
    config.C_Ch = get_input(f"C_Ch [current={config.C_Ch}]: ", config.C_Ch, float)
    config.n_qubits = max(2, get_input(f"Number of qubits [current={config.n_qubits}]: ",
                                        config.n_qubits, int))
    config.k_grid_size = get_input(f"K-grid size (Chern) [current={config.k_grid_size}]: ",
                                    config.k_grid_size, int)
    config.lyapunov_time = get_input(f"Lyapunov time [current={config.lyapunov_time}]: ",
                                      config.lyapunov_time, float)
    config.permutation_n = get_input(f"Permutation N [current={config.permutation_n}]: ",
                                      config.permutation_n, int)
    config.output_dir = input(f"Output directory [current={config.output_dir}]: ") or config.output_dir
    config.plot_dpi = get_input(f"Plot DPI [current={config.plot_dpi}]: ", config.plot_dpi, int)
...
def show_configuration(config):
    ...
    print(f"  Lattice size:        {config.lattice_size}×{config.lattice_size}")
    print(f"  Number of vortices:  {config.n_vortices}")
    print(f"  Alpha:               {config.alpha}")
    print(f"  Disorder W:          {config.disorder_strength}")
    print(f"  C_Ch:                {config.C_Ch}")
    print(f"  Number of qubits:    {config.n_qubits}")
    print(f"  K-grid size:         {config.k_grid_size}")
    print(f"  Lyapunov time:       {config.lyapunov_time}")
    print(f"  Permutation N:       {config.permutation_n}")
...
AB-CLOUD VORTEX SYSTEM FOR N-BODY PROBLEM (Python v2.0)

This code implements the vortex model of the three-body (and N-body) problem
based on the AB-cloud methodology with Chaplygin topological integral.
...
def custom_n_vortex_simulation(config, N_v):
    """Run custom simulation with N_v vortices."""
    print(f"\n--- CUSTOM {N_v}-VORTEX SIMULATION ---")
    cfg = Config()
    ...
    print(f"Building Hamiltonian (L={cfg.lattice_size}, N_v={N_v})...")
    H = build_hofstadter_hamiltonian(cfg)
    ...
    print(f"  Verdict: {rmt.verdict}")

    if config.generate_plots:
        plot_dir = ensure_report_dir(config)
        generate_plot_spacing_distribution(rmt, N_v, plot_dir, config.plot_dpi)
        print(f"Plot saved to: {plot_dir}/spacing_distribution_Nv{N_v}.png")
...
def verify_analytical_solution(config):
    ...
    if config.generate_plots:
        ...
        print(f"\nPlot saved to: {plot_dir}/analytical_solution_CCh{config.C_Ch:.2f}.png")
...
def main():
    ...
    print("  Version 2.0 — Extended with 12 new verification modules")
    print("=" * 70)
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                Nv = int(input("Enter N_v (3, 4, 5, ...): "))
                custom_n_vortex_simulation(config, Nv)
            elif choice == "10":
                print("\n--- GENERATE REPORTS ---")
                report_data = {"config": {"C_Ch": config.C_Ch, "n_vortices": config.n_vortices}}
                plot_dir = ensure_report_dir(config)
