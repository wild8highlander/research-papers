"""
    English translation of generate_report_v2.py.
...
def build_report():
    ...
    summary_text = (
        ...
        "notлandnotйbutwithть), проinедеbut 20 чandwithленных эtowithперandменthenin with пwithеintospectrumльным "
        ...
        "лучше дandwithwithandпатandinных моделей. Эthat result обобщён on mKdV, BBM and "
        "equation Каinахары (§16.23). В §16.24 реалfromоinаon fromоspectral "
        "модandфandtoацandя b via/through flow K₂ KdV-andерархandand — gauge "
        "transformation, withохраняющее spectrum Лаtowithа with precision O(θ²). В §16.25 "
        "by/ontoаforbut, that this tohe/itwithтруtoцandя глубоtoо withinяforon with classandчеwithtoой "
        "реnormлandforцandей Уandлwithshe/it (β-function ↔ K₂-flow, μ ↔ θ_b). Вwithе 25 "
        "tohe/itwiththatнт monograph inерandфandцandроinаны."
    )
...
def build_report_part5(doc):
    ...
    rows_16_10 = [
        ...
        ["generate_report.py", "Generation DOCX fromчёthat", "≈ 1550 withтроto"],
        ["Вwithhis/its", "Полonя verification + fromчёт", "≈ 5720 withтроto"],
    ]
    add_table(doc,
