"""
generate_report — English Version
============================================================

Generate comprehensive verification report with figures and statistics (version 1).

This is the English translation of generate_report.py.
Russian comments in the code body are preserved for reference.

Original file: generate_report.py
"""
...
def build_report():
    ...
    summary_text = (
        ...
        "нелинейность), проведено 15 численных экспериментов с псевдоспектральным "
        "методом 4-го порядка (N=1024, T=50). Показано, что механизм M2 "
        "(Родригес) сохраняет инварианты КдФ (M, P, E) с точностью 10⁻⁴ — "
        "на два порядка лучше диссипативных моделей. Все 25 констант "
        "монографии верифицированы (максимум остатка < 10⁻³). Результат "
        "подтверждает структурные свойства b (ортогональность, отсутствие "
        "диссипации) и расширяет Теорему 13.1 до 8 поверхностей."
    )
...
def build_report_part5(doc):
    ...
    rows_16_10 = [
        ["kdv_core.py", "Ядро: IFRK4, 3 b-механизма, 5 моделей", "≈ 400 строк"],
        ...
        ["generate_report.py", "Генерация DOCX отчёта", "≈ 700 строк"],
        ["Всего", "Полная верификация + отчёт", "≈ 3170 строк"],
    ]
    add_table(doc,
              ["Файл", "Описание", "Объём"],
              rows_16_10,
