function results = ab_cloud_verify_en(varargin)
% ==============================================================================
% AB-Cloud Verification Suite — MATLAB (English)
% ==============================================================================
% English-only version. For bilingual, use ab_cloud_verify.m
%
% Usage:
%   results = ab_cloud_verify_en()
%   results = ab_cloud_verify_en('dataDir', '../data', 'zeros', 50000, ...
...
% ==============================================================================
    ...
    gammas = load_zeros_en(dataDir, nZeros, src, MSG);
    if isempty(gammas)
        fprintf('%s\n', MSG.no_data); results = []; return;
    end

    results = struct();
    if ismember(obj, {'all', '1'}), results.obj1 = obj1_en(gammas, MSG); end
    if ismember(obj, {'all', '2'}), results.obj2 = obj2_en(gammas, MSG); end
    if ismember(obj, {'all', '3'}), results.obj3 = obj3_en(gammas, MSG); end
    fprintf('\n%s\n%s\n\n', MSG.separator, MSG.done);
end

% --- Load zeros (English helper) ---------------------------------------------
function gammas = load_zeros_en(dataDir, count, source, MSG)
    files = struct( ...
    ...
    if isempty(selected) || ~exist(selected,'file'), error('No data file found in: %s',dataDir); end
    fprintf(MSG.loading+'\n', selected);
...
% --- Gram point --------------------------------------------------------------
function g = gram_point_en(n)
    if n<=0, g=0; return; end
    g = 2*pi*n/lambert_W0_en(n/exp(1));
    for iter=1:3
        theta=0.5*g*log(g/(2*pi))-0.5*g-pi/8; dtheta=0.5*log(g/(2*pi));
        g=g+(pi*n-theta)/dtheta;
    end
end
function gv = gram_vec_en(nv)
    gv=zeros(size(nv)); for i=1:numel(nv), gv(i)=gram_point_en(nv(i)); end
end

% --- Objection 1 -------------------------------------------------------------
function res = obj1_en(gammas, MSG)
    N=numel(gammas);
    ...
    for ci=1:numel(cp)
        idx=1:cp(ci); gram=gram_vec_en(idx'); bN=mean(abs(gammas(idx)-gram));
...
% --- Objection 2 -------------------------------------------------------------
function res = obj2_en(gammas, MSG)
    N=numel(gammas);
    ...
    pv=max(0,min(1,pv));
    fprintf('\n%18s: %.8f\n','D-statistic',D); fprintf('%18s: %.6e\n','p-value',pv);
    if pv>0.05, v=MSG.obj2_pass; else, v=MSG.obj2_fail; end
    fprintf('\n%s\n',v); res=struct('D',D,'pvalue',pv);
end

% --- Objection 3 -------------------------------------------------------------
function res = obj3_en(gammas, MSG)
    N=numel(gammas);
    fprintf('\n%s\n%s\n%s\n%s\n',MSG.separator,MSG.obj3_title,MSG.obj3_desc,MSG.separator);
    st=max(1,floor(N*0.5)); idx=st:N; gram=gram_vec_en(idx'); dev=abs(gammas(idx)-gram);
