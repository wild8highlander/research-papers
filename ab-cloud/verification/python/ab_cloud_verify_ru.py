"""
AB-Cloud Verification Suite — Russian-Only Interface.

Предоставляет ту же логику проверки, что и ab_cloud_verify, но со всеми
строками вывода на русском языке.  Используйте этот модуль, когда нужен
явно русский вывод независимо от переменной LANG.
"""
...
# ---------------------------------------------------------------------------
# Русский каталог сообщений (фиксированный)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Функция Ламберта W (главная ветвь)
# ---------------------------------------------------------------------------

def lambert_w(x: float, tol: float = 1e-12, max_iter: int = 50) -> float:
    """Вычислить главную ветвь W₀(x) методом Ньютона."""
    if x < -1.0 / math.e:
        raise ValueError(f"Функция Ламберта не определена для x = {x} < −1/e")
    if x == 0.0:
        return 0.0
...
def gram_point(k: int) -> float:
    """Вычислить k-ю точку Грама: γ̃_k ≈ 2πk / W(k/e)."""
    if k <= 0:
        return 0.0
...
def _parse_plain_zeros(lines: List[str]) -> List[float]:
    """Разобрать нули из простого текста (пропустить комментарии и пустые строки)."""
    zeros: List[float] = []
    for line in lines:
        s = line.strip()
...
def _parse_highT_zeros(lines: List[str]) -> List[float]:
    """Разобрать формат high-T блоков."""
    zeros: List[float] = []
    base_offset = 0.0
    for line in lines:
        s = line.strip()
...
def _parse_csv_zeros(lines: List[str]) -> List[float]:
    """Разобрать CSV-формат со столбцами: index,t,s_real,s_imag,zero_number."""
    zeros: List[float] = []
...
def _parse_julia_zeros(lines: List[str]) -> List[float]:
    """Разобрать формат массива Julia."""
    zeros: List[float] = []
    inside_array = False
    for line in lines:
        s = line.strip()
...
def load_zeros(
    ...
    """Загрузить нули дзета-функции Римана из файлов данных (русский вывод).

    Параметры
    ---------
    data_dir : str или None
        Путь к каталогу data/. По умолчанию ../data/ относительно этого файла.
    count : int
        Количество нулей для загрузки (по умолчанию 5000).
    source : str
        Имя источника данных: "auto", "50k", "500k", "2M", "highT", "2M_gz", "csv", "zeros6".

    Возвращает
    ---------
    list[float]
        Мнимые части нулей дзета-функции, отсортированные по возрастанию.
    """
    if data_dir is None:
...
def compute_bN(zeros: List[float], N: int) -> float:
    """Вычислить b(N) = (1/N) Σ_{k=1}^{N} |γ_k − γ̃_k|."""
...
def verify_objection1(
    ...
    """Проверить возражение 1: сходимость b(N)."""
    if N_values is None:
    ...
    if plot:
        try:
            ...
            plt.close(fig)
            print("  → График сохранён: objection1_convergence.png")
        except ImportError:
            pass
...
def _cramer_von_mises_test(empirical: List[float], cdf_func) -> float:
    """Статистика Крамера-фон Мизеса W²."""
    n = len(empirical)
    if n == 0:
        return 0.0
    sorted_data = sorted(empirical)
...
def verify_objection2(zeros: List[float]) -> Dict[str, object]:
    """Проверить возражение 2: статистическая значимость GUE-распределения."""
    print(f"\n{'='*60}")
    ...
    print(f"  Вычислено {len(spacings)} нормализованных интервалов")
    ...
    try:
        ...
        ax.hist(spacings, bins=80, density=True, alpha=0.6, color="#2563eb",
                edgecolor="white", linewidth=0.5, label="Эмпирическое")
        s_range = [i * 0.02 for i in range(500)]
        gue_vals = [gue_pdf(s) for s in s_range]
        ax.plot(s_range, gue_vals, "r-", linewidth=2, label="GUE (Вигнер-Дайсон)")
        ax.set_xlabel("Нормализованный интервал s")
        ax.set_ylabel("Плотность")
        ax.set_title("Распределение GUE-интервалов")
        ax.legend()
        ax.grid(True, alpha=0.3)
        fig.savefig("objection2_gue_spacing.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
        print("  → График сохранён: objection2_gue_spacing.png")
    except ImportError:
        pass
...
def _linear_regression(x: List[float], y: List[float]) -> Tuple[float, float, float, float]:
    """Линейная регрессия y = slope·x + intercept. Возвращает (slope, intercept, std_err, R²)."""
    n = len(x)
    if n < 2:
...
def verify_objection3(zeros: List[float]) -> Dict[str, object]:
    """Проверить возражение 3: b(N) = O(1/√N) — скорость убывания."""
    print(f"\n{'='*60}")
    ...
    print("  Вычисление b(N) для нескольких значений N …")
    for N in N_vals:
        bN = compute_bN(zeros, N)
        if bN <= 0:
            continue
    ...
    if len(log_N) < 3:
        print("  ВНИМАНИЕ: Недостаточно точек для регрессии")
    ...
    try:
        ...
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.scatter(log_N, log_bN, color="#2563eb", s=30, zorder=5, label="Данные")
        fit_x = [min(log_N), max(log_N)]
        fit_y = [slope * x + intercept for x in fit_x]
        ax.plot(fit_x, fit_y, "r--", linewidth=2, label=f"Аппрокс.: наклон = {slope:.4f}")
        ref_y = [-0.5 * x + intercept for x in fit_x]
        ax.plot(fit_x, ref_y, "g:", linewidth=1.5, label="Эталон: наклон = −0.5")
        ax.set_xlabel("log(N)")
        ...
        plt.close(fig)
        print("  → График сохранён: objection3_decay_rate.png")
    except ImportError:
        pass
...
def run_verification(
    ...
    """Запустить все или выбранные возражения (русский вывод)."""
    if zeros is None:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="AB-Cloud: Проверка (русский)")
