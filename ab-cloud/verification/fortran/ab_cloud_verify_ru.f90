!===============================================================================
! AB-CLOUD ПРОВЕРОЧНЫЙ ПАКЕТ — РУССКИЙ
...
contains
  ...
  subroutine load_zeros(gammas, n_loaded, n_request, source_name)
    ...
    write(*,'(A,A)') "  Файл данных: ", trim(filepath)

    open(newunit=u, file=trim(filepath), status='old', action='read', iostat=ios)
    if (ios /= 0) then; write(*,'(A,A)') "  ОШИБКА: Не удалось открыть ", trim(filepath); stop 1; end if
  ...
  function lambert_w0(x) result(w)
    real(dp), intent(in) :: x; real(dp) :: w; integer :: iter
    if (x == 0.0_dp) then; w = 0.0_dp; return; end if
    w = log(max(x, 1.0e-30_dp)); if (w > 0.0_dp) w = log(w)
    do iter = 1, 50
      block
        real(dp) :: ew, f, fp, fpp, delta
        ew = exp(w); f = w*ew - x; fp = ew*(w+1.0_dp); fpp = ew*(w+2.0_dp)
        delta = f / (fp - 0.5_dp * f * fpp / fp); w = w - delta
  ...
  function gram_point(n) result(gn)
    integer(int64), intent(in) :: n; real(dp) :: gn
  ...
  subroutine compute_objection1(gammas, n, b_n, pass)
    ...
    real(dp), intent(out) :: b_n; logical, intent(out) :: pass
    real(dp) :: s; integer(int64) :: k
    s = 0.0_dp
    do k = 1, n; s = s + abs(gammas(int(k)) - gram_point(k)); end do
    b_n = s / real(n, dp); pass = (b_n < 1.0_dp)
    write(*,'(/,A)')  "═══════════════════════════════════════════════════"
  ...
  subroutine compute_objection2(gammas, n, ks_stat, ks_pval, pass)
    real(dp), intent(in) :: gammas(:)
    integer(int64), intent(in) :: n
    real(dp), intent(out) :: ks_stat, ks_pval; logical, intent(out) :: pass
    real(dp), allocatable :: s(:), sorted_s(:)
    real(dp) :: log_factor, cdf_theo, cdf_emp, d_plus
    integer(int64) :: k, m
    m = n - 1; allocate(s(int(m)))
    do k = 1, m
    ...
    ks_stat = 0.0_dp
    do k = 1, m
      cdf_emp = real(k, dp) / real(m, dp); cdf_theo = gue_cdf(sorted_s(int(k)))
      d_plus = abs(cdf_emp - cdf_theo); if (d_plus > ks_stat) ks_stat = d_plus
    end do
    ks_pval = kolmogorov_pvalue(ks_stat, real(m, dp)); pass = (ks_pval > 0.05_dp)
    write(*,'(/,A)')  "═══════════════════════════════════════════════════"
...
end program ab_cloud_verify_ru
